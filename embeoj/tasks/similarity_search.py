...
index_path = os.path.join(GLOBAL_CONFIG["CHECKPOINT_DIRECTORY"], GLOBAL_CONFIG["PROJECT_NAME"],SIMILARITY_SEARCH_CONFIG["INDEX_FILE_NAME"])
...
def get_checkpoint_version():
    try:
        checkpoint_version_file = os.path.join(GLOBAL_CONFIG["CHECKPOINT_DIRECTORY"], GLOBAL_CONFIG["PROJECT_NAME"],"checkpoint_version.txt")
...
def read_embeddings(entity_file):
    try:
        ...
        embeddings_path = os.path.join(GLOBAL_CONFIG["CHECKPOINT_DIRECTORY"], GLOBAL_CONFIG["PROJECT_NAME"],embedding_file)
...
def return_entity_index(entity_id):
    try:
        data_directory = os.path.join(GLOBAL_CONFIG["DATA_DIRECTORY"], GLOBAL_CONFIG["PROJECT_NAME"])
        ...
        entity_files = [re.findall("entity_names_[a-zA-Z]*[_][0-9]{1,2}.json", filename) for filename in os.listdir(data_directory)]
        ...
        for entity_file in entity_files:
            ...
            if query_entity_index is not None:
                ...
                logging.info(f"entity found in {entity_file} at index {query_entity_index}")
